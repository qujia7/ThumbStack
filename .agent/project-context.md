...
- **Catalog prep:** `prepare_catalogue.py`, `catalogue_merged.py`; `bgs/prepare_catalogue_bgs.py` (BGS BRIGHT-20.2 with recon velocities)
...
### Running (2026-03-12) — superseded by wide-prior runs below
- Tight-prior HOD-marginalized MCMC for all bins submitted (jobs 1168402/1168404/1168405/1168407)
  - Configs: `/home/jiaqu/ksz_mcmc/tight_priors_hod_marg/ksz_bin{1,2,3}.yaml`, `ksz_all.yaml`
  - Output: `output/zbin{1,2,3}_tight_hod_marg`, `output/zbin_all_tight_hod_marg` (converged chains — used as covmats below)

### Ready to submit (2026-03-15) — widened priors, covmats wired in
- **Set A (emulator):** `/home/jiaqu/ksz_mcmc/tight_priors_hod_marg/ksz_bin{1,2,3}.yaml`, `ksz_all.yaml`
  - Wide priors: log10_A_rho0 [2.1, **4.5**], A_beta [1.7, **6.0**], a_k2h [0, 4]
  - Ref locs: log10_A_rho0=**3.50**, A_beta=**4.00**
  - Covmats: `/home/jiaqu/ksz_mcmc/output/zbin{1,2,3}_tight_hod_marg.covmat` / `zbin_all_tight_hod_marg.covmat`
  - Output: `output/zbin{1,2,3}_hod_marg_wide`, `output/zbin_all_hod_marg_wide`; resume=False
- **Set B (no emulator):** `/home/jiaqu/ksz_mcmc/tight_priors/ksz_mcmc{,2,3}.yaml`, `ksz_all.yaml`
  - Same prior/ref changes; covmat entries unchanged
  - Output: `output/zbin{1,2,3}_fixed_hod_wide`, `output/zbin_all_fixed_hod_wide`; resume=False

### Submitted (2026-03-17) — v2 fixed-HOD runs for bins 1 and 3 with wide-run covmats
- **zbin1_fixed_hod_v2** (job 1190609): `tight_priors/ksz_mcmc_v2.yaml`, script `cobaya_fixed_hod_bin1_v2.sh`
...
- **zbin2_fixed_hod_v3** (job 1190641, resubmit after covmat fix): `tight_priors/ksz_mcmc2_v3.yaml`, script `cobaya_fixed_hod_bin2_v3.sh`
  - covmat: `output/zbin2_ksz900_7000_namaste_new_HOD.covmat` (switched from contaminated v2 covmat); output: `output/zbin2_fixed_hod_v3`; resume=False
  - Tighter priors: log10_A_rho0 max 4.5→3.8, A_beta max 5.0→4.2, A_beta ref loc 3.50→3.00
  - job 1190630 crashed: v2 covmat had island contamination (log10_A_rho0 std=0.53, A_beta std=0.70)
...
## IMPORTANT: Outstanding Investigation

- **GP emulator z2 (bin2, z_eff=0.725) fix applied** (2026-03-11): The bin2 emulator was incorrectly trained using the z2 HOD chain (`LRG_z2_all_chains.txt`) and z3+z4 combined dndz. Fixed `gp_emulator.py` to use the z1 chain (`LRG_z1_all_chains.txt`) and bin2 dndz (`HOD/dndz_bin2_interpolated_new.txt`). Retraining submitted via `sbatch gp_emulator.sh`. Output will overwrite `/scratch/jiaqu/HOD/gp_emulator_2d_z0.725_.pkl`. After completion, verify HOD bounds match z1 chain (sigma_logM ~0.01-0.06, kappa ~0.54-0.87).
- **GP emulator z3 (z_eff=0.950) verified correct** (2026-03-11): bin3 uses z2 chain as intended. The identical HOD bounds between the old bin2 and bin3 emulators were because bin2 was mistakenly using the z2 chain — not because bin3 was wrong.

---

## Decisions Log
- Use r=0.65 from Hadzhiyska et al. 2024 light cone analysis (LRG)
...
## BGS Analysis Parameters
- Tracer: BGS BRIGHT-20.2
- z_eff = 0.26 (mean redshift of full sample)
- r = 0.64 (velocity correlation coefficient — DIFFERENT from LRG r=0.65, must use this for BGS theory/fitting)
- Catalog: `bgs/prepare_catalogue_bgs.py` produces 5 subsamples (full, logm>9.5, >10.0, >10.5, >11.0)
- vR already provided as reconstructed LOS velocity in km/s (no pre/post-rec redshift differencing needed)
- Recon settings: R=12.5 Mpc/h, nmesh=512, recsym, MG
