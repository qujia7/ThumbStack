...
Uses BINARY mask option:
  - Template: template_car_local_nbar.fits
  - Mask: template_car_local_nbar_binary_mask.fits (binary footprint)

Based on namaste_car.py but uses the local nbar normalized template.

Output: cl_master_local_nbar.txt
"""
...
p = argparse.ArgumentParser(description="NaMaster cross-spectra with local nbar template (binary mask)")
...
print("="*60)
print("NaMaster cross-spectra: LOCAL NBAR + BINARY MASK")
print("="*60)
...
# Load LOCAL NBAR template and binary mask
template = enmap.read_map(f'{args.output_dir}/../stage_template/template_car_local_nbar.fits')
binary_mask = enmap.read_map(f'{args.output_dir}/../stage_template/template_car_local_nbar_binary_mask.fits')

# Use binary mask for the galaxy field (same approach as original, different template)
combined = cmb_mask_binary * binary_mask

wcs = combined.wcs

print(f"[info] Template: template_car_local_nbar.fits")
print(f"[info] Mask: binary (template_car_local_nbar_binary_mask.fits)")
...
# Save the workspace
wsp_file = f'{args.output_dir}/../stage_spectra/workspace_cmb_template_local_nbar.fits'
...
# Save results
output_file = f'{args.output_dir}/../stage_spectra/cl_master_local_nbar.txt'
np.savetxt(output_file,
           np.column_stack([ell_eff, cl_deconvolved]),
           header="ell Cl_CMB_x_template (local nbar, binary mask)")
...
np.savetxt(f'{args.output_dir}/../stage_spectra/error_local_nbar.txt', error)
print(f"Error saved to: {args.output_dir}/../stage_spectra/error_local_nbar.txt")

print("\n" + "="*60)
print("DONE: cl_master_local_nbar.txt")
print("="*60)
