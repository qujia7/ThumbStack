...
#         self.EstMBins = [] #['ksz_uniformweight'] #['tsz_uniformweight', 'ksz_uniformweight']
      ...
      # Output path
      self.pathOut = os.path.join(workDir, "output/"+self.name)
      comm = MPI.COMM_WORLD
      rank = comm.Get_rank()
      ...
      # Ensure all processes wait until rank 0 has created the directory
      comm.Barrier()

      # Figures path
      self.pathFig = os.path.join(workDir, "figures/"+self.name)
      ...
      # Test figures path
      self.pathTestFig = os.path.join(self.pathFig)
      if rank == 0:
...
#            self.saveAllStackedMaps(filterTypes=self.filterTypes, Est=self.Est)
   ...
   def runEndToEnd(self, test):
      ...
      #in this step shuffling do not enter
      if self.save:
         self.saveFiltering(nProc=self.nProc, test=test)
...
#      stampHit[:,:] = self.cmbHit.at(ipos, prefilter=True, mask_nan=False, order=3)
   ...
   def saveFiltering(self,nProc=1, test=False):
      ...
      # Distribute the total number of objects among MPI tasks.
      # Each process will get a list (my_tasks) of object indices to process.
      comm, rank, my_tasks = mpi.distribute(self.Catalog.nObj)

      # Each MPI process computes its own local results.
      # We include the object index so that later the results can be reassembled in order.
      local_results = [(iObj, self.analyzeObject(iObj, test=test)) for iObj in my_tasks]

      # Gather the results from all MPI tasks.
      # This returns a list (one per process) of the local_results lists.
      all_results = comm.allgather(local_results)

      # Only the rank-0 process will merge and save the results.
      if rank == 0:
         # Merge the results into a dictionary keyed by the object index.
         all_res_dict = {}
         for sublist in all_results:
               for iObj, res in sublist:
                  all_res_dict[iObj] = res

         # Reassemble the full list of results in the proper order.
         results = [all_res_dict[iObj] for iObj in range(self.Catalog.nObj)]
         tStop = time()
         print("took", (tStop-tStart)/60., "min")

         # Loop over each filter type and save the corresponding arrays.
         for filterType in self.filterTypes:
               filtMap = np.array([results[iObj][0][filterType][:] for iObj in range(self.Catalog.nObj)])
               filtMask = np.array([results[iObj][1][filterType][:] for iObj in range(self.Catalog.nObj)])
               filtHitNoiseStdDev = np.array([results[iObj][2][filterType][:] for iObj in range(self.Catalog.nObj)])
               filtArea = np.array([results[iObj][3][filterType][:] for iObj in range(self.Catalog.nObj)])
   ...
   def measureVarFromHitCount(self, filterType,  plot=False):
      ...
      mask = self.catalogMask(overlap=False, psMask=True, filterType=filterType,
...
#      tMean = ts.meanT[filterType].copy()
 ...
 #     tMean = tMean[mask,:]
      ...
      v -= np.mean(v) #there is a mean velocity subtraction
...
#          # print('Tlarge:', Tlarge)
      ...
      if iBootstrap is not None:
         ...
         if block_bootstrap:
            print("block bootstraping")
            ra = self.Catalog.RA   # in deg
            dec = self.Catalog.DEC   # in deg
            ra_filtered = ra[mask == 1]
            dec_filtered = dec[mask == 1]
            ra_edges = np.arange(0, 360 + deg_bootstrap, deg_bootstrap)   # edges from 0 to 360 degrees
            dec_edges = np.arange(-20, 21, deg_bootstrap)  # if you only want to define bins for -20 to 20

            # Digitize the ra and dec values to get bin indices.
            # np.digitize returns the bin index each value falls into.
            ra_bins = np.digitize(ra_filtered, ra_edges)  
            dec_bins = np.digitize(dec_filtered, dec_edges)

            # Combine the ra and dec bin indices to hash a unique block index.
            block_idx = ra_bins * 1000 + dec_bins
            unique_blocks, block_idx_relabel = np.unique(block_idx, return_inverse=True)
            n_blocks = len(unique_blocks)
            nObj = np.sum(mask)
            boot_indices_list = []
            counter = 0

            # We'll repeatedly sample blocks until we reach at least nObj indices.
            while counter < nObj:
               # Randomly choose one block (with replacement)
               b = np.random.choice(np.arange(n_blocks))
               indices_in_block = np.where(block_idx_relabel == b)[0]
               
               # Check if adding the entire block would exceed nObj
               if counter + len(indices_in_block) > nObj:
                  # Determine how many indices we need to exactly reach nObj
                  needed = nObj - counter
                  boot_indices_list.append(indices_in_block[:needed])
                  counter += needed
               else:
                  boot_indices_list.append(indices_in_block)
                  counter += len(indices_in_block)

            # Concatenate the list into a single array
            boot_indices = np.concatenate(boot_indices_list)
      
            
            # Now use these boot_indices to resample your data arrays:
 
            t = t[boot_indices, :]
            v = v[boot_indices]
            s2Hit = s2Hit[boot_indices, :]
            s2Full = s2Full[boot_indices, :]
      ...
      # kSZ: uniform weighting
         ...
         print("##########################")
         print("uniform weight kSZ")
         # remove mean temperature
         #t -= np.mean(t, axis=0)
#         t -= tMean
         #weights here are one
...
#         return resMap
   ...
   def plotFilterHistograms(self, filterType, mVir=None, z=[0., 100.], ts=None):
      ...
      # select objects that overlap, and reject point sources
      mask = ts.catalogMask(overlap=False, psMask=True, filterType=filterType, mVir=mVir, z=z)
   ...
   def SaveCovBootstrapStackedProfile(self, filterType, est, mVir=None, z=[0., 100.], nSamples=100, nProc=1):
      ...
      Uses MPI (via orphics.mpi) for parallelization.
      """
      ...
      # If more than one process is to be used, distribute the tasks with MPI:
      from orphics import mpi
      comm, rank, my_tasks = mpi.distribute(nSamples)
      # Each MPI task computes its assigned bootstrap sample(s)
      local_results = [self.computeStackedProfile(filterType, est, iBootstrap=i, mVir=mVir, z=z)
                        for i in my_tasks]
      ...
      if rank == 0:
         print("took", (tStop - tStart)/60., "min")
         # Unpack the results (assuming computeStackedProfile returns a tuple where the first element is the profile)
         stackSamples = result[:, 0, :]  # shape (nSamples, nRAp)
         np.savetxt(self.pathOut + "/stackSamples_" + filterType + "_" + est + "_bootstrap.txt", stackSamples)

         # Estimate covariance matrix across the samples (columns = radial bins)
   ...
   def SaveCovVShuffleStackedProfile(self, filterType, est, mVir=None, z=[0., 100.], nSamples=100, nProc=1):
      ...
      This version uses MPI (via orphics.mpi) to distribute nSamples among tasks.
      """
      print("#############################################")
      print("doing shuffling using MPI")
      print("this is the correct call")
      from time import time
      tStart = time()
      ...
      # Distribute the total number of samples among MPI tasks.
      # This returns (comm, rank, my_tasks) where my_tasks is a list of sample indices.
      from orphics import mpi
      comm, rank, my_tasks = mpi.distribute(nSamples)
      
      # Each process computes its assigned samples:
      local_results = [self.computeStackedProfile(filterType, est, 
                                                   iVShuffle=i, 
                                                   mVir=mVir, z=z, 
                                                   iBootstrap=2)
                        for i in my_tasks]
      ...
      all_results = comm.allgather(local_results)
      # Flatten the gathered list (list of lists) into a single list.
      results_list = []
      for sub in all_results:
         results_list.extend(sub)
      result = np.array(results_list)
      ...
      if rank == 0:
         print("took", (tStop-tStart)/60., "min")
         # Unpack results. Here we assume that computeStackedProfile returns a tuple
         # where the first element is the stacked profile.
         stackSamples = result[:, 0, :]  # shape (nSamples, nRAp)
         #also save the raw samples
         np.savetxt(self.pathOut + "/stackSamples_" + filterType + "_" + est + "_vshuffle.txt", stackSamples)

         # Estimate the covariance matrix (columns represent radial bins)
         covStack = np.cov(stackSamples, rowvar=False)
         # Save the covariance matrix.
         np.savetxt(self.pathOut + "/cov_" + filterType + "_" + est + "_vshuffle.txt", covStack)
         
         # Also perform a null test: compute the mean and error on the mean.
         data = np.zeros((self.nRAp, 3))
         data[:, 0] = self.RApArcmin
         data[:, 1] = np.mean(stackSamples, axis=0)  # mean of the shuffled stacks
         data[:, 2] = np.sqrt(np.diag(covStack)) / np.sqrt(nSamples)
   ...
   def SaveCovBootstrapTwoStackedProfiles(self, ts2, filterType, est, mVir=None, z=[0., 100.], nSamples=100, nProc=1):
      ...
      mask1 = self.catalogMask(overlap=False, psMask=True, filterType=filterType, mVir=mVir, z=z)
      mask2 = ts2.catalogMask(overlap=False, psMask=True, filterType=filterType, mVir=mVir, z=z)
      mask12 = mask1 * mask2
      n1 = np.sum(mask1)
      n2 = np.sum(mask2)
      n12 = np.sum(mask12)
   ...
   #    if nProc > 1:
   #       with sharedmem.MapReduce(np=nProc) as pool:
   #          f = lambda iSample: self.computeStackedProfile(filterType, est, iVShuffle=iSample, mVir=mVir, z=z, iBootstrap=2)
   #          result = np.array(pool.map(f, list(range(nSamples))))
   #    else:
   #       result = np.array([self.computeStackedProfile(filterType, est, iVShuffle=iSample, mVir=mVir, z=z, iBootstrap=2)
   ...
   def saveAllStackedMaps(self, filterTypes=None, Est=None):
      ...
      for filterType in filterTypes:
         ...
         for est in Est:
            ...
            # stackedMap = self.computeStackedProfile(filterType, est, iBootstrap=None, iVShuffle=None, tTh='', stackedMap=False)
...
#         self.plotFilterHistograms(filterType)
         ...
         # for ksz only
         if self.doVShuffle:
            print("computing the covmat of the shuffling velocities")
            # for est in self.estVShuffle:
            for est in self.EstBootstrap:
            # for est in ['ksz_uniformweight']:
   ...
   def loadAllStackedProfiles(self, theory=True):
...
#       ax.set_ylabel(r'$\sigma(T)$ [$\mu K\cdot\text{arcmin}^2$]')
   ...
   def plotAllStackedProfiles(self):
      ...
      for filterType in self.filterTypes:
         # all stacked profiles
         for est in self.Est:
            self.plotStackedProfile(filterType, [est], name=filterType+"_"+est, theory=True)
         ...
         if self.doMBins:
            ...
            for est in self.EstMBins:
               ...
               self.plotStackedProfile(filterType, estArr, name=filterType+"_"+est+"_mmax", theory=True, legend=False)
               # expected from tSZ
               estArr = [est+"_mmax"+str(iMMax)+"_theory_tsz" for iMMax in range(self.nMMax)] + [est]
               self.plotStackedProfile(filterType, estArr, name=filterType+"_"+est+"_mmax_theory_tsz", theory=True, legend=False)
               # expected from kSZ
               estArr = [est+"_mmax"+str(iMMax)+"_theory_ksz" for iMMax in range(self.nMMax)] + [est]
               self.plotStackedProfile(filterType, estArr, name=filterType+"_"+est+"_mmax_theory_ksz", theory=True, legend=False)
   ...
   def computeAllSnr(self, theory=True):
